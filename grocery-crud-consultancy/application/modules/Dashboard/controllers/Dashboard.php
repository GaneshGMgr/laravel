  ...
  public function changeUserPassword(){
    ...
    if(isset($_POST['change_password'])){
      ...
      if($this->form_validation->run($this) === FALSE) {
        ...
        $getCurrPass = $this->db->query("SELECT pass FROM user_auth WHERE user_id = '$empId' AND is_active = 'Active' AND is_locked_flag = 'Not-Locked'")->row_array();

        if ($getCurrPass['pass']==md5($this->input->post('curr_password'))) {
          ...
          if ($this->input->post('curr_password') != $this->input->post('confirm_password')) {
            ...
            if ($checkPassword['response']) {
              ...
              $this->db->update('user_auth', $pass, array('user_id' => $empId));
